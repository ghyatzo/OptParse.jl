...
- `tryoptparse(parser, argv)` / `tryoptparse(::Type{T}, argv)`
- `optparse(parser, argv)` / `optparse(::Type{T}, argv)`
- `runparse(parser, argv; ...)` / `runparse(::Type{T}, argv; ...)`
- `partial(parser)` + `tryoptparse(partial(p), argv)`
- `lift(::Type{T})` — access the parser for a type generated by [`@parser`](@ref)
